...
const ShowSemesterGoals = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://127.0.0.1:8000/api/class/lastest-semester/${class_id}`)
      .then((response) => {
        console.log("API dataaaaaa:", response);
        setSemesterId(response.semesterId);
      })
      .catch((error) => {
  ...
  const fetchGoals = async (semesterId) => {
    ...
    try {
      ...
      console.log("API dataeeee:", response.data.data); // kiểm tra dữ liệu
    } catch (err) {
